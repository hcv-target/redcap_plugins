...
$date_fields = array('dm_usubjid', 'dm_rfstdtc', 'dm_rfendtc', 'eot_dsterm', 'dis_dsstdy', 'hcv_suppfa_fuelgbl', 'hcv_suppfa_nlgblrsn', 'hcv_suppfa_hcvout', 'hcv_suppfa_wk10rna', 'hcv_suppfa_lastbloq', 'dis_suppds_funcmprsn', 'hcv_suppfa_fudue', 'dm_suppdm_hcvt2id', 'dm_actarmcd', 'dm_suppdm_rtrtsdtc');
$date_data = REDCap::getData('array', $subjects, $date_fields, $first_event_id);
$blip_threshold = 500;
...
foreach ($date_data AS $subject_id => $subject) {
	...
	foreach ($subject AS $date_event_id => $date_event) {
		...
		$stop_date = isset($date_event['dm_rfendtc']) && $date_event['dm_rfendtc'] != '' ? $date_event['dm_rfendtc'] : null;
		...
		if ($t3_fk_result) {
			...
			$t3_start_date_value = get_single_field($t3_fk['record'], $project_id, $first_event_id, 'dm_rfstdtc', '');
		}
		 ...
		 */
		update_field_compare($subject_id, $project_id, $first_event_id, $re_treat_date, $date_event['dm_suppdm_rtrtsdtc'], 'dm_suppdm_rtrtsdtc', $debug);
	}
}
